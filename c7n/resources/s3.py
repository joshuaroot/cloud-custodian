...
class Lifecycle(BucketActionBase):
    ...
    def process_bucket(self, bucket):
        ...
        if 'get_bucket_lifecycle_configuration' in bucket.get(
                'c7n:DeniedMethods', []):
            log.warning(
                "Access Denied Bucket:%s while reading lifecycle" % bucket[
                    'Name'])
            return
        ...
        try:
            s3.put_bucket_lifecycle_configuration(Bucket=bucket['Name'], LifecycleConfiguration={'Rules': config})
        except ClientError as e:
            if e.response['Error']['Code'] == 'AccessDenied':
                log.warning(
                    "Access Denied Bucket:%s while applying lifecycle" % bucket[
                        'Name'])
            else:
                raise e
